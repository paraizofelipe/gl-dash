...
The second key defines an exact mapping between the `dlvhdr/gl-dash` repository and the
`~/code/gl-dash` folder. Because this is a non-wildcard mapping, the dashboard will use
this value to resolve the repository path for `dlvhdr/gl-dash` even though there's also
an entry for `dlvhdr/*`.
...
a wildcard. If a key ends with a wildcard but the value doesn't, `gl-dash` won't be
able to correctly map repositories to folders.

### Without a Wildcard

If the key for an `repoPath` entry doesn't have a wildcard (`*`), its value must not
have a wildcard. If a key ends without a wildcard but the value does, `gl-dash` won't
