...
and preview pane display for the views, use the [`defaults`], [`mrSections`], and
...
| Property | Type | default                                   |
| :------- | :--- | :---------------------------------------- |
| `author` | color | `#5fafff` for dark or `#5fafff` for light |

This setting determines the color of the text for the following elements in the
dashboard UI:

- The username of the author, in the pull requests and issues tables, for both the
  compact and the default (extended) row layouts
