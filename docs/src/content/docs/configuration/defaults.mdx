...
these settings in any section you define in the [`mrSections`] or [`issueSections`] settings.

They determine which columns are displayed and how.

[`mrSections`]: /configuration/pr-section
...
the width is `6`, `gl-dash` displays as `gl-d`.
...
| Type   |              Options              | Default |
| :----- | :-------------------------------: | :-----: |
| String | "notifications", "mrs", "issues" |  "mrs"  |
...
This setting specifies whether the user needs to confirm when quitting `gl-dash`
When this is on, `gl-dash` shows a prompt that requires the user to press <kbd>y</kbd>/<kbd>Enter</kbd> to actually quit.
