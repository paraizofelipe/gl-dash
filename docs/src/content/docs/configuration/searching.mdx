...
In addition to GitLab's filters, gl-dash adds templating functions.
...
You can configure short aliases for long project paths and use them in the `project:`
(or `repo:`) filter as `project:@alias/rest`. See [Project Aliases](/configuration/project-aliases)
for details.
