...
keybindings:
  prs:
    - key: H
      ...
      command: >
        ~/code/personal/gl-dash/scripts/herdr-mr-review.sh {{.RepoPath}} {{.PrNumber}}
```
...
The script behind the keybinding, `scripts/herdr-mr-review.sh`, is idempotent: it reuses or creates a `code-review` herdr workspace and a `MR-<PrNumber>` tab, then runs `claude "/mr-review <PrNumber>" --dangerously-skip-permissions` in that tab. Pressing `H` again for the same PR just focuses the existing tab instead of starting a new session.
